...
<head>
    ...
    <title>Tempo || Input Order TempoTV</title>
    <style>
        body {
...
<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Input Order TempoTV :') }} {{ $k_order }}
        </h2>
    </x-slot>
...
<div class="container">
    ...
    <div class="flex flex-row">
        ...
        <div class="flex justify-center">
          <div class="mb-3 xl:w-96">
              <label
                for="day_off" class="form-label inline-block mb-2 text-gray-700 text-sm mr-3">
                Jumlah Day Off :
              </label>
              <input
                type="number"
                class="form-control block w-full px-2 py-1 text-sm font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300
                  rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none"
                  name="day_off" id="day_off" placeholder="Day Off"/>
              <label style="font-size: 10px;">Wajib di isi, isi 0 jika tidak ada day off.</label>
          </div>
        </div> 
      ...
      <div class="py-4 inline-block min-w-full sm:px-6 lg:px-8">
        <div class="overflow-hidden">
          <table class="min-w-full text-center">
            ...
            <tbody>
              @foreach($iklan_tempo_tv as $iklantempotv)
              <tr class="bg-white border-b">
                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                  {{ $iklantempotv->nik }}
                </td>
                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                  {{ $iklantempotv->k_order }}
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                  {{ $iklantempotv->judul }}
                </td>
                <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                  <a href="javascript:void(0)" id="btn-detail-omtimbm" data-id="{{ $iklantempotv->nik }}"
    ...
    $(function(){
        ...
        $("#ket_muat_order").change(function(){     
            ...
            if(jk == '1') {
                ...
                $('#periode_order_start').prop('disabled', true);
                $('#periode_order_akhir').prop('disabled', true);
            } else {
                $('#tgl_order').prop('disabled', true);
                $('#periode_order_start').prop('disabled', false);
                $('#periode_order_akhir').prop('disabled', false);
            }
        });
        ...
        $("#day_off").change(function(){
            var startDate = $('#periode_order_start').val();
            var endDate = $('#periode_order_akhir').val();
            var dayOff = $('#day_off').val();
  ...
  <script>
    ...
    $('#saveorderkorantempo').click(function(e) {
      ...
      $.ajax({
          ...
          url: `/mediaorder/saveordertempotv`,
          type: "POST",
          cache: false,
          data: {
              "k_order": k_order,
